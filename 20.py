...
class Zefoy:
	...
	def get_captcha(self):
		...
		try:
			...
			print(f"[hoàng tool]</> Không thể giải captcha: {e}")
			time.sleep(2)
			self.get_captcha()
	...
	def send_captcha(self, new_session = False):
		...
		if 'Enter Video URL' in request.text: 
			print('[hoàng tool]</> Session đã được tạo')
			open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
			print(f"[hoàng tool]</> Giải capcha thành công: {captcha_solve}")
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[Nguyên]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return
				...
				elif lamtilo >= 1000: print('[Nguyên]</> Your IP was banned')
				# print(f'[hoàng tool]</> Thời gian chờ để tiếp tục tăng: {t} giây')
				# for lam in range(t,0,-1):
				# 	lam-=1
				# 	print(f"[hoàng tool]</> Vui lòng chờ: {lam}",end="\r")
				# 	time.sleep(1)
				_=time.time()
				while time.time()-2<_+lamtilo:
					t-=1
					print("[hoàng tool]</> Vui lòng chờ: {0} ".format(t)+"giây .", end="\r")
					time.sleep(1)
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[hoàng tool]</> Link video không hợ lệ'); return False
...
def logo():
    ...
    {xanh_la} </> Nguyễn minh hoàng + phan hưng + minh nguyễn</>
    \033[1;37m────────────────────────────────────────────────────────────
    {do}[{trang}{kt_code}{do}] {vang}TOOL BUFF TYM TIKTOK 
    {do}[{trang}{kt_code}{do}] \033[1;35mADMIN:\033[1;36m NguyễnMinh Hoàng
    {do}[{trang}{kt_code}{do}] {dac_biet}box zalo:\033[1;37m https://zalo.me/g/yleaqz769
    \033[1;37m────────────────────────────────────────────────────────────\n"""
  
    print(logo)

if 1:
    print(f"{xanh_la}[hoàng tool]</> Key đúng mời bạn vô tool")
    logo()
    Z = Zefoy()
    ...
    print(f"{do}[hoàng tool]</> Key sai vui lòng vượt lại")
    exit()
